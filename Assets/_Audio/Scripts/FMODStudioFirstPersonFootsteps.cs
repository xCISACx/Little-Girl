...
public class FMODStudioFirstPersonFootsteps : MonoBehaviour
{
    ...
    void Update() // This method, and everyting insid of it, is perfomred once every frame. If the game is running at an average of 60 frames per second, this method will be perfomred 60 times a second.
    {
        ...
        if (PlayerTouchingGround && Input.GetKeyDown(KeyCode.Space))    // If the player is touching the ground AND the player presses the button to jump at the same time, we knoe that the player character is about to jump, therefore we perform our method to play a sound.
        {
